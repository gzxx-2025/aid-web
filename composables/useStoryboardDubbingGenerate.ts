import { useStep4PlusMockGen } from '~/composables/useStep4PlusMockGen'
...
function resolveDubbingMockDelayMs(): number {
  ...
  const env = (import.meta as unknown as { env?: Record<string, string> }).env
    ?.NUXT_PUBLIC_MOCK_STEP4_PLUS_GEN
  if (env === '0' || env === 'false') {
    return 8000 + Math.floor(Math.random() * 4200)
  }
...
function resolveDubbingOutputUrl(
...
function resolveComposeProgressMessage(status: ComposeStatusResult): string {
  ...
  if (st === 'COMPOSING') return '视频合成中…'
  if (st === 'SUCCEEDED') return '合成完成'
  if (st === 'FAILED') return String(status.errorMessage || '配音生成失败')
  return '配音任务处理中…'
}
...
export async function submitStoryboardDubbingGenerateTask(
  ...
  if (useStep4PlusMockGen().enabled) {
    return { mode: 'mock' }
  }
