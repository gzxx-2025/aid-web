...
export function useVideoModelGenerateSettings(options: UseVideoModelGenerateSettingsOptions) {
  ...
  const supportsAudio = computed(() => capabilitySnapshot.value.supportsAudio)
  const audioSelectOptions = computed(() =>
    buildAudioSelectOptions(capabilitySnapshot.value.supportsAudio)
  )
