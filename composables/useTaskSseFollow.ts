...
/**
 * Pause every task SSE currently owned by the active create-flow page.
 * This only releases browser connections. Persisted task snapshots stay intact
 * so the destination page (or the same page when revisited) can restore safely.
 */
export function suspendAllTaskSseFollows(): number {
  return suspendTaskSseFollowSlots(activeTaskSseFollowSlots)
}

/** Pause one SSE follow without cancelling its server-side task. */
...
export async function waitUserTaskSseTerminal(payload: {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const benignError =
        ev.type === 'error' && isBenignTaskSseDisconnectMessage(ev.errorMessage)
...
/**
 * follow* 助手：SSE 被抢占，或良性断连且任务仍在进行中 → 调用方应静默退出，勿清 Pinia / 勿 toast。
 */
