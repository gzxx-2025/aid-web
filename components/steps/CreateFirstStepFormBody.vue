      ...
      <div class="content_box">
        ...
        <div class="field-group inline-options">
          ...
          <div v-else class="chip-group">
            <template v-for="opt in scriptTypeOptions" :key="opt.value">
              <a-tooltip v-if="isScriptTypeOptionDisabled(opt.value)" title="暂不支持">
                <span class="chip-tooltip-wrap">
                  <button
                    type="button"
                    class="chip is-disabled"
        ...
        <div class="field-group inline-options">
          ...
          <div v-else class="chip-group">
            <template v-for="opt in creationModeOptions" :key="opt.value">
              <a-tooltip
                v-if="isCreationModeOptionDisabled(opt.value)"
                title="解说漫暂不支持该创作模式"
              >
                <span class="chip-tooltip-wrap">
                  <button
...
const { ensureLoaded: ensureDictLoaded, enumList } = usePromptDictionary()
...
function pickScriptType(v: ScriptTypeValue) {
  ...
  const nextMode = resolveCreationModeForScriptType(displayCreationMode.value, v)
  if (nextMode !== displayCreationMode.value) {
    if (isNewProjectModal.value) leftPanelDraft.creationMode = nextMode
    emit('update:creationMode', nextMode)
  }
}
...
function pickCreationMode(v: CreationModeValue) {
  ...
  if (isScriptTypeLockedToPlot(v) && displayScriptType.value !== 'plot') {
    if (isNewProjectModal.value) leftPanelDraft.scriptType = 'plot'
    emit('update:scriptType', 'plot')
  }
}

function isScriptTypeOptionDisabled(value: ScriptTypeValue): boolean {
  return isScriptTypeLockedToPlot(displayCreationMode.value) && value === 'monologue'
}

function isCreationModeOptionDisabled(value: CreationModeValue): boolean {
  return isCreationModeDisabledForScriptType(value, displayScriptType.value)
}
...
watch(
  [() => props.creationMode, () => creationModeOptions.value, () => displayScriptType.value],
  ([current, options, scriptType]) => {
    if (props.flowEditMode) {
      ensureEnumSelection(current, options, (v) => emit('update:creationMode', v), normalizeCreationModeValue)
    }
    const resolved = resolveCreationModeForScriptType(current, scriptType)
    if (resolved !== current) {
      if (isNewProjectModal.value) leftPanelDraft.creationMode = resolved
      emit('update:creationMode', resolved)
      return
    }
    if (isScriptTypeLockedToPlot(current) && displayScriptType.value !== 'plot') {
      if (isNewProjectModal.value) leftPanelDraft.scriptType = 'plot'
      emit('update:scriptType', 'plot')
    }
  },
...
.right-panel :deep(.my-styles .subsection-title.title-one) {
  display: none;
}

.right-panel :deep(.my-styles) {
  margin-bottom: 8px;
}

.right-panel :deep(.featured-styles--collapsed) {
  flex-shrink: 0;
}

.right-panel :deep(.featured-styles--collapsed .styles-grid) {
  overflow: visible;
  padding-bottom: 4px;
}
