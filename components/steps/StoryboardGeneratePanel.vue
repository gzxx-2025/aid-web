<template>
  <div
    ...
    :style="panelCssVars"
  >
    <div v-if="showScriptFileHeader" ref="headerWrapRef">
    ...
    >
      <div class="storyboard-left-col">
        <div class="asset-panel">
          ...
          <template v-if="mode === 'storyboardVideo' && showStoryboardVideoAssets">
    ...
    >
      ...
      <GenerateSourceImagesStrip
        ...
        :show-adder="storyboardVideoReferenceList.length < 1"
        :show-adder-text="true"
        adder-text="导入参考图"
        @remove="onStoryboardVideoStripRemove"
        @preview="onStoryboardVideoReferenceThumbClick"
        @open-adder="emit('importReference')"
      />
      <!-- 多参生视频：场景/角色/道具参考图导入区 -->
      <GenerateSourceImagesStrip
        v-else-if="mode === 'imageToVideo'"
        :key="'i2v-multi-param'"
        variant="i2v"
        :use-shimmer="true"
        :enable-preview="true"
        ...
        :show-adder-text="true"
        adder-text="导入参考图"
        @remove="onMultiParamStripRemove"
        @preview="emit('previewAssetImage', $event)"
        @open-adder="emit('importReference')"
      />
      <!-- 首尾帧：文本域上方仅展示参考音频（首尾帧图在 prompt-prefix） -->
      <GenerateSourceImagesStrip
        v-else-if="mode === 'edgeVideo' && referenceAudioStripItems.length > 0"
        :key="'i2v-edge-audio'"
        variant="i2v"
        :use-shimmer="true"
        :enable-preview="true"
        ...
        :show-adder="false"
        @remove="emit('removeReferenceAudio', $event)"
      />
      <RichTextEditor
      ...
      <div class="prompt-footer prompt-footer-modal">
        ...
        <div v-if="mode !== 'storyboardVideo' && mode !== 'edgeVideo'" class="prompt-footer-right">
    ...
    <StoryboardParamSettingsModal
      v-if="useParamSettingsModal && mode !== 'storyboardVideo' && mode !== 'edgeVideo'"
...
const props = withDefaults(
  defineProps<{
    mode: 'storyboard' | 'imageToVideo' | 'storyboardVideo' | 'edgeVideo'
  ...
  }>(),
  {
    ...
    suppressPromptReactiveSync: false,
    showStoryboardVideoAssets: true,
    referenceImages: () => [],
    referenceAudios: () => [],
...
function syncStoryboardVideoPromptWithoutImageRefs() {
  ...
  const prevPlain = storyboardPromptHtmlToPlain(props.prompt)
...
watch(
  ...
  () =>
    nextTick(() => {
      if (props.suppressPromptReactiveSync) return
...
const referenceAudioStripItems = computed(() =>
  (props.referenceAudios ?? []).map((a, i) => ({
    ...a,
    kind: 'audio' as const,
    id: a.id ?? `audio-${i}`,
...
/** 图生/宫格：图片 + 音频混排素材条 */
const storyboardVideoStripItems = computed(() => [
  ...storyboardVideoReferenceList.value.map((img) => ({ ...img, kind: 'image' as const })),
  ...referenceAudioStripItems.value
])

/** 多参：资产图 + 音频混排素材条 */
const multiParamStripItems = computed(() => [
  ...multiParamAssetReferenceList.value.map((img) => ({ ...img, kind: 'image' as const })),
  ...referenceAudioStripItems.value
])
...
function onStoryboardVideoStripRemove(index: number) {
  const imgCount = storyboardVideoReferenceList.value.length
  if (index < imgCount) {
    emit('removeReferenceImage', index)
    return
  }
  emit('removeReferenceAudio', index - imgCount)
}

function onMultiParamStripRemove(index: number) {
  const imgCount = multiParamAssetReferenceList.value.length
  if (index < imgCount) {
    emit('removeMultiParamAssetReference', index)
    return
  }
...
.asset-reference-item .shimmer-image {
  width: 100%;
  height: 100%;
}
...
.asset-thumb-item .shimmer-image {
  width: 100%;
  flex-shrink: 0;
}
