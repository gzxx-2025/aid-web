  ...
  >
    ...
    <div class="music-picker-shell">
      ...
      <div class="music-picker-footer">
        ...
        <button
          type="button"
          class="music-picker-upload"
          :disabled="uploading"
          @click="triggerUpload"
        >
          <CloudUploadOutlined class="music-picker-upload-icon" />
          <span class="music-picker-upload-text">{{ uploading ? '上传中…' : '上传本地音频' }}</span>
        </button>
        <input
          ref="fileInputRef"
...
.music-picker-upload:hover:not(:disabled) {
  border-color: rgba(74, 231, 253, 0.45);
  background: rgba(74, 231, 253, 0.06);
}

.music-picker-upload:disabled {
  opacity: 0.55;
  cursor: not-allowed;
  pointer-events: none;
}
