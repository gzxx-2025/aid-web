<template>
  <a-modal
    ...
    @cancel="handleCancel"
  >
    <div class="select-scene-shell">
      <!-- 头部：标题和场景切换器 -->
      <header class="modal-header">
        <div class="header-top">
          <ModalTitleWatermark :title="title" watermark="IMPORT" />
          <button type="button" class="select-scene-close" aria-label="关闭" @click="handleCancel">
            <CloseOutlined />
          </button>
        </div>
        <!-- 场景切换器：显示所有场景，当前场景高亮 -->
              ...
              >
                <div class="image-wrapper">
                  <ShimmerImage
                    ...
                    wrapper-class="scene-image-shimmer"
                    object-fit="cover"
                  ...
                  <div v-else class="image-placeholder">
                    <img
                      :src="noDataIconUrl"
                      alt=""
                      class="empty-image-icon empty-image-icon--md"
                    />
                    <p>暂无图片</p>
                  </div>
                </div>
...
import { UploadOutlined, FolderOutlined, CloseOutlined } from '@ant-design/icons-vue'
...
.image-wrapper :deep(.shimmer-image) {
  width: 100%;
  height: 100%;
}

.image-wrapper :deep(.shimmer-image__img.scene-image) {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
...
<style lang="scss">
.ant-modal-wrap.select-scene-image-modal-wrap .ant-modal.select-scene-image-modal .ant-modal-content {
  padding: 0 !important;
  overflow: hidden;
  border-radius: 12px;
  background: #191a1d;
...
.ant-modal-wrap.select-scene-image-modal-wrap .ant-modal.select-scene-image-modal .ant-modal-header {
  display: none !important;
}

.ant-modal-wrap.select-scene-image-modal-wrap .ant-modal.select-scene-image-modal .ant-modal-body {
  padding: 0 !important;
  background: #191a1d;
}
</style>
