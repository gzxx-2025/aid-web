<template>
  ...
  <AgentPickerModal
    ...
    :func-code="agentPickerFuncCode"
    :model-type="agentPickerModelType"
...
const videoModelOptionsResolved = computed((): VideoModelOption[] =>
  modelTypeEnumOptions.value.map((e) => ({
    id: e.value,
    name: e.label,
    desc: '',
    icon: ViduIcon,
...
const agentPickerDefaultQuery = computed(() => {
  if (agentPickerContext.value === 'video') return '导演'
  return '分镜'
})

const agentPickerFuncCode = computed(() => {
  if (agentPickerContext.value === 'script') return STORYBOARD_SCRIPT_MODEL_FUNC_CODE
  if (agentPickerContext.value === 'video') return AI_MODEL_FUNC_CODE.STORYBOARD_VIDEO_PROMPT
  return ''
})

const agentPickerModelType = computed((): AiModelType | undefined => {
  if (agentPickerContext.value === 'video') return 'text'
  return 'text'
})
...
async function loadScriptAgents(autoPickDefault = false) {
  ...
  try {
    ...
    scriptAgentOptions.value = agentOptionsFromGroup(groups, STORYBOARD_SCRIPT_AGENT_BIZ_CATEGORY)
...
async function loadVideoAgents(autoPickDefault = false) {
  ...
  try {
    ...
    videoAgentOptions.value = agentOptionsFromGroup(groups, STORYBOARD_VIDEO_PROMPT_AGENT_BIZ_CATEGORY)
...
const localSoundEffects = ref<SoundEffectsOption>(props.soundEffects ?? 'with-sound')
  ...
  (v) => {
    if (v) {
      if (props.source === 'script') {
        ...
        localVideoModel.value = props.videoModel ?? 'vidu-q2-pro'
        localAspectRatio.value = props.aspectRatio ?? '16:9'
        localResolution.value = props.resolution ?? '720p'
        localSoundEffects.value = props.soundEffects ?? 'with-sound'
...
watch(
  ...
  ([opts]) => {
    ...
    localVideoModel.value = snapToFirstOption(localVideoModel.value, ids, 'vidu-q2-pro')
  },
  { flush: 'post' }
)
