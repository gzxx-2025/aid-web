<template>
  ...
  >
    <template #title>
      <ModalTitleWatermark title="生成配置" watermark="CONFIGURATION" />
    </template>
                    ...
                    >
                      <div class="pgc-agent-model-card__agent">
                        ...
                        <div class="pgc-agent-model-card__info">
                          <div class="pgc-agent-model-card__name">
                            {{ agentDisplayName(scene.sceneCode) }}
                          </div>
                          <div class="pgc-agent-model-card__desc">
                            {{ agentDisplayDesc(scene.sceneCode) }}
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="pgc-agent-model-card__model">
                        <div
                          class="pgc-agent-model-card__avatar pgc-agent-model-card__avatar--model"
                        >
                          <img
                            v-if="modelDisplayLogo(scene.sceneCode)"
                            :src="modelDisplayLogo(scene.sceneCode)"
                            :alt="modelDisplayName(scene.sceneCode)"
                            class="pgc-agent-model-card__avatar-img"
                          />
                          <span v-else>{{ modelDisplayInitial(scene.sceneCode) }}</span>
                        </div>
                        <div class="pgc-agent-model-card__info">
                          <div class="pgc-agent-model-card__name">
                            {{ modelDisplayName(scene.sceneCode) }}
                          </div>
                          <div class="pgc-agent-model-card__desc">
                            {{ modelDisplayProvider(scene.sceneCode) }}
                          </div>
                        </div>
                      </div>
                    </button>
...
interface SceneRowDraft {
  ...
  resolution: string
  aspectRatio: string
  /** project=用户已保存；default/none=矩阵/空，清晰度比例应以模型 capability 默认为准 */
  source: string
...
const applicableSceneCodes = ref<string[]>([])

const visibleSceneGroups = computed(() =>
  buildProjectGenConfigVisibleGroups(applicableSceneCodes.value)
)
...
async function seedSceneRowDefaults(sceneCode: string, kind: ProjectGenConfigSceneKind) {
  ...
  // source=project：保留用户已保存的清晰度/比例；否则（矩阵默认）强制用当前模型 capability 默认值
  // 避免场景级 resolution=2K 与模型 defaultSize=1K 不一致时页面展示错误
  const forceModelDefaults = String(cur.source || '').trim() !== 'project'
  rows[sceneCode] = applyImageParamDefaults(
    sceneCode,
    kind,
    {
      agentCode,
...
async function loadAll() {
  ...
  try {
    ...
    applicableSceneCodes.value = sceneCodes
...
async function handleSave() {
  ...
  const configs = visibleSceneGroups.value
    .flatMap((g) => g.scenes)
    .map((scene) => {
...
watch(
  () => [props.open, props.projectId, props.episodeId] as const,
  ([open]) => {
