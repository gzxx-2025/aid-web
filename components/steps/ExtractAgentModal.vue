<template>
  <a-modal
    ...
    :wrap-class-name="extractWrapClassName"
    @cancel="handleCancel"
  >
    <template #title>
    ...
    <div class="extract-footer">
      ...
      <template v-if="actionMode === 'continueOrReextract'">
        <a-button
          type="primary"
          size="large"
        ...
        <a-tooltip title="重新提取会将之前提取的内容删除并重新计费">
          <span class="extract-btn-reextract-wrap">
            <a-button
              size="large"
              class="extract-btn-reextract"
              :disabled="isExtracting || !genConfigReady"
...
const extractWrapClassName = computed(() =>
  props.scope === 'all'
...
const userPickedModelByKind = reactive<Record<AgentKind, boolean>>({
  scene: false,
  character: false,
  prop: false
})

const genConfigReady = ref(false)
...
function applyGenConfigForKind(
  kind: AgentKind,
  cfg: import('~/types/business-api').ProjectGenConfigVO | null,
  ...
  if (autoPickDefault) {
    ...
    if (agentCode) {
      ...
      if (first) {
        agentCode = first.id
        localAgents[kind] = { ...first }
      }
    }
    const modelCode = pickProjectGenModelCodeFromVo(cfg, agentCode, agents)
...
const handlePicked = (payload: { agent?: AgentOption; modelCode?: string }) => {
  ...
  if (payload.modelCode) {
    ...
    userPickedModelByKind[pickerKind.value] = true
  }
  emit('update:agents', {
    scene: { ...localAgents.scene },
    character: { ...localAgents.character },
...
const handleStart = (overwrite = false) => {
  if (isExtracting.value || !genConfigReady.value) return
  ...
  const payload = {
    ...
    manualModelPickByKind: {
      scene: userPickedModelByKind.scene,
      character: userPickedModelByKind.character,
      prop: userPickedModelByKind.prop
    },
    overwrite: !!overwrite
  }
...
watch(
  ...
  (open) => {
    if (!open) {
      ...
      progressPercent.value = 0
      userPickedModelByKind.scene = false
      userPickedModelByKind.character = false
      userPickedModelByKind.prop = false
...
.extract-btn-reextract:hover:not(:disabled) {
