...
async function claimFormImagesFromMatchingTerminalTasks(generatingFormIds: Set<number>) {
  ...
  const ctx = await resolveStoryScriptSaveContext(creationStore, route)
  if (!ctx) return
  const allRows = await fetchFlowUserTaskList(ctx.projectId, { intent: 'read' }).catch(
    () => [] as UserTaskRow[]
  )
 ...
 */
async function reloadOngoingTasks(options?: {
  force?: boolean
  mutate?: boolean
}): Promise<boolean> {
  ...
  reloadOngoingTasksPromise = (async (): Promise<boolean> => {
    ...
    try {
      ...
      const allRows = await fetchFlowUserTaskList(ctx.projectId, {
...
<style lang="scss" scoped src="~/assets/css/scp-scene-character-prop.scoped.scss"></style>
