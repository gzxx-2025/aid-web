...
提示:若为全集/多集内容,请按集拆分后分别创建作品导入"
        min-height="500px"
        :max-length="scriptMaxLength"
        count-pure-text-only
        show-count
...
const scriptMaxLength = computed(() =>
...
const handleImport = (content: string) => {
  ...
  if (pureLen > scriptMaxLength.value) {
    message.warning(
      `导入内容已超过字数上限（${scriptMaxLength.value.toLocaleString('zh-CN')}字），请删减后再导入`
    )
    return
  }
  localContent.value = content
