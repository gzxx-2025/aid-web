<template>
  <a-modal
    ...
    wrap-class-name="create-flow-modal storyboard-param-settings-modal-wrap"
    destroy-on-close
    @cancel="handleCancel"
  >
              ...
              <div class="spsm-readonly-block">
                <div class="spsm-field">
                  <div class="spsm-switch-row">
                    ...
                    <a-switch :checked="draftNineGridEnabled" :disabled="true" size="small" />
                  </div>
                </div>
                <div class="spsm-field">
            ...
            <template v-else-if="mode === 'imageToVideo'">
              ...
              <div class="spsm-field">
                ...
                <SettingSelectField
                  ...
                  @update:open="
                    (v) => (draftI2vActiveVideoSettingKey = v ? 'cameraMovement' : null)
                  "
                />
                <div class="spsm-textarea-wrap">
                  <a-input
              ...
              <div class="spsm-field">
                ...
                <SettingSelectField
                  ...
                  @update:open="
                    (v) => (draftI2vActiveVideoSettingKey = v ? 'shootingTechnique' : null)
                  "
                />
              </div>
            </template>
...
type AssetCategoryKey = 'scene' | 'character' | 'prop' | 'pose' | 'expression' | 'effect' | 'draft'
...
const enableDraftPromptAssetRefs = computed(
  () =>
    props.mode === 'storyboard' || props.mode === 'imageToVideo' || props.mode === 'storyboardVideo'
)
...
const draftPromptAssets = computed<PromptAssetItem[]>(() => {
  ...
  return props.extraPromptAssets?.length ? mergePromptAssets(props.extraPromptAssets, local) : local
})
...
function initDraftFromProps() {
  ...
  draftI2vNineGridEnabled.value =
    props.imageToVideoNineGridEnabled ?? props.nineGridEnabled ?? false
  draftI2vReferenceImages.value = cloneList(
    props.imageToVideoReferenceImages?.length ? props.imageToVideoReferenceImages : refsFromProps
  )
  ...
  draftSelectedComposition.value = props.selectedComposition
    ? { ...props.selectedComposition }
    : null
  draftSelectedShotSize.value = props.selectedShotSize ? { ...props.selectedShotSize } : null
  draftSelectedCameraAngle.value = props.selectedCameraAngle
    ? { ...props.selectedCameraAngle }
    : null
  draftSelectedFocalLength.value = props.selectedFocalLength
...
function buildConfirmPayload(): ParamSettingsConfirmPayload {
  ...
  return {
    ...
    referenceImages,
    selectedComposition: draftSelectedComposition.value
      ? { ...draftSelectedComposition.value }
      : null,
    selectedShotSize: draftSelectedShotSize.value ? { ...draftSelectedShotSize.value } : null,
    selectedCameraAngle: draftSelectedCameraAngle.value
      ? { ...draftSelectedCameraAngle.value }
      : null,
    selectedFocalLength: draftSelectedFocalLength.value
      ? { ...draftSelectedFocalLength.value }
      : null,
...
<style lang="scss">
.ant-modal-wrap.storyboard-param-settings-modal-wrap .ant-modal.storyboard-param-settings-modal .ant-modal-content {
  padding: 0 !important;
  overflow: hidden;
...
.ant-modal-wrap.storyboard-param-settings-modal-wrap .ant-modal.storyboard-param-settings-modal .ant-modal-body {
  padding: 0 !important;
}
</style>
