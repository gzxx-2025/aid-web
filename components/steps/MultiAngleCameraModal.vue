<template>
  <a-modal
    ...
    :title="null"
    :closable="false"
    wrap-class-name="create-flow-modal multi-angle-modal-wrap"
    class="multi-angle-modal"
  >
    <div class="ma-shell">
      <header class="ma-header">
        <h2 class="ma-title">多机位</h2>
        <button type="button" class="ma-close" aria-label="关闭" @click="modalOpen = false">
          <CloseOutlined />
        </button>
      </header>
...
const isFlatPreviewState = computed(
  () =>
    !fixedNineGrid.value &&
    !hasInteracted.value &&
...
const showFlatOverlay = computed(
  () =>
    ...
    (!textureReady.value || isFlatPreviewState.value)
)
...
const updateCameraRig = () => {
  ...
  if (isFlatPreview) {
    ...
    rigGroup.rotation.set(0, 0, 0)
    if (texturedSphereMaterial) {
      const uniforms = texturedSphereMaterial.uniforms as Record<string, { value: THREE.Vector2 }>
...
.multi-angle-layout {
  --ma-preview-max-w: min(82%, 720px);
  --ma-preview-max-h: min(72%, 480px);
  display: flex;
  flex-direction: row;
  align-items: stretch;
  flex: 1 1 auto;
...
.import-btn-dashed :deep(.anticon) {
  display: inline-flex;
  line-height: 0;
}
