      ...
      <div :class="['generate-settings-params', `generate-settings-params--cols-${paramColumns}`]">
        <template v-if="mode === 'video'">
          ...
          <div v-if="showDuration" class="setting-item">
            ...
            <div class="setting-duration-select-wrap">
              <a-tooltip :title="durationTip || undefined" placement="top">
                <a-select
          ...
          <div v-if="showAudio" class="setting-item">
...
const videoAudioSelectOptions = computed(() => {
  ...
  if (props.videoAudioOptions != null) return props.videoAudioOptions
  return DEFAULT_VIDEO_AUDIO_OPTIONS
})
...
const audio = defineModel<string>('audio', { default: 'with_audio' })
</script>
...
.setting-duration-select-wrap {
  display: block;
  width: 100%;
  min-width: 0;
}

/* a-tooltip 可能包一层 inline 节点，强制与 ant-select 同宽撑满栅格列 */
.setting-duration-select-wrap :deep(> span),
.setting-duration-select-wrap :deep(> div) {
...
.setting-duration-select-wrap :deep(.ant-select) {
  width: 100%;
}
