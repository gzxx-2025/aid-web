...
const { modelList: modelOptions, rawModelList, loadModels: loadGenerationModelOptionsFallback } =
...
const {
  ...
  loadModels: loadDialogueModelOptionsFallback
} = useModelList<ModelOption>({
...
async function initImageModelOptions() {
  ...
  if (!applySceneModalImageModelPool(modelGroups, assetType)) {
    try {
      const list = await userModelListByFunc(formImageBiz, listScope)
      if (list.length > 0) {
        rawModelList.value = list
        modelOptions.value = list.map(mapSceneModalModelItem)
      } else {
        await loadGenerationModelOptionsFallback()
      }
    } catch {
      await loadGenerationModelOptionsFallback()
    }
  ...
  if (!applySceneModalDialogueModelPool(modelGroups)) {
    await loadDialogueModelOptionsFallback()
  }
...
.history-list {
  ...
  min-height: 0;
  flex: 1;
  /* 与 EditStoryboardVideoModal 一致：20px 左右 padding 会在 144px 栏宽下撑出横向滚动条 */
  padding: 0 12px 10px;
...
.history-item :deep(.history-item__image) {
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
...
.canvas-toolbar :deep(.ant-tooltip-open) {
  flex: 1 1 0;
  min-width: 0;
  display: flex;
}

.canvas-toolbar :deep(.upscale-model-popover-root),
.canvas-toolbar :deep(.setting-card-popover-root) {
  flex: 1 1 0;
  min-width: 0;
  width: 100%;
  display: flex;
...
.canvas-toolbar :deep(.upscale-model-popover-trigger),
.canvas-toolbar :deep(.setting-card-popover-trigger) {
  flex: 1;
  width: 100%;
  display: flex;
  justify-content: center;
...
.canvas-toolbar :deep(.ant-btn .ant-btn-icon) {
  margin-inline-end: 0 !important;
  margin-inline-start: 0 !important;
  line-height: 0;
}
...
.canvas-toolbar :deep(.ant-btn:hover:not(:disabled)) {
...
.canvas-toolbar :deep(.ant-btn.toolbar-tab-hover .ant-btn-icon) {
  color: #fff !important;
}
...
.figma-stage-layout .canvas-toolbar :deep(.ant-btn-text.toolbar-tab-hover),
.figma-stage-layout .canvas-toolbar :deep(.ant-btn-text.toolbar-tab-hover > span),
.figma-stage-layout .canvas-toolbar :deep(.ant-btn-text.toolbar-tab-hover .ant-btn-icon) {
  color: #fff !important;
}

/* 与 EditStoryboardImageModal .storyboard-canvas-meta 同位置、同尺寸 */
.scene-canvas-meta {
  display: flex;
  align-items: center;
...
.edit-scene-image-modal .stage-config-panel :deep(.ql-editor.ql-blank::before) {
