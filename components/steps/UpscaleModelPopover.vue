          ...
          <template v-else-if="modelOptions.length">
            <div class="upscale-model-list">
              <button
                ...
                @click.stop="pick(opt)"
              >
                <div class="upscale-model-thumb" aria-hidden="true">
                  <img
                    v-if="opt.icon"
                    :src="opt.icon"
                    :alt="opt.name"
                    class="upscale-model-thumb__img"
                  />
                  <span v-else class="upscale-model-thumb__letter">{{
                    (opt.name || '?').slice(0, 1)
                  }}</span>
                </div>
                <div class="upscale-model-body">
                  <div class="upscale-model-title-row">
                    ...
                    <span v-if="opt.resolutionLabel" class="upscale-model-tag">{{
                      opt.resolutionLabel
                    }}</span>
                  </div>
...
function mapModelRow(item: UserModelListItem): UpscaleModelOption | null {
  ...
  if (!code) return null
  const resolution = resolveUpscaleResolutionFromModel(item, props.resolutionFormat)
...
async function loadOptions() {
  const prefetched = props.prefetchedModels
