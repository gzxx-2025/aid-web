<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <span
          v-if="item.assetType === 'audio'"
          class="prompt-asset-ref-picker__thumb prompt-asset-ref-picker__thumb--audio"
          aria-hidden="true"
        />
        <img
          v-else-if="item.url"
      ...
      <div v-if="!assets.length" class="prompt-asset-ref-picker__empty">
...
const assetsAreAudio = computed(() => props.assets.some((a) => a.assetType === 'audio'))
</script>
...
.prompt-asset-ref-picker__thumb--audio {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  background: var(--accent-100, rgba(74, 231, 253, 0.1));
  border: 1px solid var(--accent-400, rgba(74, 231, 253, 0.42));
  &::after {
    content: '';
    width: 18px;
    height: 12px;
