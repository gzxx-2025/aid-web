<template>
  ...
  <a-modal
    ...
    wrap-class-name="recharge-modal-wrap"
    :z-index="1000"
    @cancel="handleClose"
  >
    <template #title>
    ...
    <div class="recharge-modal__body recharge-modal__body--step1">
      ...
      <div v-if="!anyPaymentEnabled" class="recharge-modal__pay-off" role="status">
        支付通道暂未开放，请稍后再试
      </div>
      ...
      <footer class="recharge-modal__footer-bottom">
        <div class="recharge-modal__footer-links">
          ...
          <button type="button" class="recharge-modal__footer-link" @click="showConsume = true">
            消耗明细
          </button>
        </div>
    ...
    <a-drawer
      ...
      width="520"
      :mask-closable="true"
      root-class-name="recharge-order-drawer"
    ...
    <a-drawer
      v-model:open="showConsume"
      title="消耗明细"
      width="520"
      :mask-closable="true"
      root-class-name="recharge-order-drawer recharge-consume-drawer"
      class="recharge-order-drawer recharge-consume-drawer"
    >
      <p class="consume-drawer__hint">按任务聚合展示积分变动，单位：Credits</p>
      <div ref="consumeScrollRef" class="consume-drawer__scroll" @scroll="onConsumeScroll">
        <div v-if="consumeLoading && !consumeRows.length" class="order-empty">加载中…</div>
        <div
          v-else-if="consumeLoadError"
          class="order-empty consume-drawer__error"
        >
          加载失败，请稍后重试
        </div>
        <div v-else-if="consumeIsEmpty" class="order-empty">暂无消耗记录</div>
        <TransitionGroup v-else name="consume-item" tag="div" class="consume-list">
          <article
            v-for="row in consumeRows"
            :key="row.bizTraceId"
            class="consume-card"
          >
            <header class="consume-card__head">
              <span class="consume-card__type">{{ row.bizTypeName || row.bizType }}</span>
              <time class="consume-card__time">{{ row.createTime || '—' }}</time>
            </header>
            <div class="consume-card__amount-row">
              <span
                class="consume-card__amount"
                :class="{
                  'consume-card__amount--consume': row.changeAmount < 0,
                  'consume-card__amount--refund': row.changeAmount > 0,
                  'consume-card__amount--neutral': row.changeAmount === 0
                }"
              >
                {{ formatConsumeChange(row.changeAmount) }}
              </span>
              <span class="consume-card__amount-label">{{ consumeAmountLabel(row) }}</span>
            </div>
            <p v-if="row.bizName" class="consume-card__biz-name">{{ row.bizName }}</p>
            <p v-if="row.modelName" class="consume-card__model">模型：{{ row.modelName }}</p>
            <ul v-if="consumeDetailItems(row).length" class="consume-card__details">
              <li v-for="(item, idx) in consumeDetailItems(row)" :key="idx">{{ item }}</li>
            </ul>
          </article>
        </TransitionGroup>
        <InfiniteScrollLoadFooter
          :loading="consumeLoadingMore"
          :has-more="consumeHasMore"
          :has-items="consumeRows.length > 0"
          end-text="已加载全部记录"
        />
      </div>
    </a-drawer>
  ...
  <a-modal
    ...
    wrap-class-name="recharge-modal-wrap recharge-pay-modal-wrap"
  ...
  >
    <div class="recharge-pay-modal__body">
      ...
      <div class="recharge-pay-modal__pay-row">
        ...
        <div v-if="anyPaymentEnabled" class="recharge-pay-modal__methods">
          <button
            v-if="alipayEnabled"
            type="button"
            :class="[
        ...
        <p v-else class="recharge-pay-modal__pay-unavailable">暂无可用支付方式，请稍后再试</p>
      </div>

      <p v-if="anyPaymentEnabled" class="recharge-pay-modal__lead-hint">
...
function resolveActivePayType(): 'alipay' | 'wxpay' | null {
  if (payType.value === 'alipay' && alipayEnabled.value) return 'alipay'
  if (payType.value === 'wxpay' && wxpayEnabled.value) return 'wxpay'
  return defaultPayType.value
}

function ensurePayTypeSynced() {
  const next = resolveActivePayType()
  if (next) payType.value = next
}
...
const consumeScrollRef = ref<HTMLElement | null>(null)
const {
  items: consumeRows,
...
function formatConsumeChange(changeAmount: number) {
...
function consumeAmountLabel(row: CreditConsumeRecordRow) {
...
function consumeDetailItems(row: CreditConsumeRecordRow): string[] {
  const items: string[] = []
...
async function syncQrForSelection() {
  ...
  ensurePayTypeSynced()
  const channel = resolveActivePayType()
  if (!channel) {
...
watch(showConsume, async (open) => {
  if (open) {
    await reloadConsumeList()
    await nextTick()
    onConsumeScroll()
  } else {
    unbindConsumeScroll()
...
.recharge-pay-modal__pay-unavailable {
  margin: 0;
  font-size: 13px;
  color: rgba(255, 180, 120, 0.95);
}
...
.consume-drawer__scroll {
  max-height: calc(100vh - 160px);
  overflow-y: auto;
  padding-right: 4px;
}

.consume-drawer__error {
  color: #fca5a5;
}

.consume-item-enter-active,
.consume-item-leave-active {
  transition: all 0.28s ease;
}

.consume-item-enter-from,
.consume-item-leave-to {
  opacity: 0;
...
.consume-card__amount-row {
  display: flex;
  align-items: baseline;
  gap: 8px;
}

.consume-card__amount {
  font-size: 20px;
  font-weight: 600;
  line-height: 1.2;
}
...
/* 订单管理 / 消耗明细抽屉：关闭图标在深色头上需为白色 */
.recharge-order-drawer .ant-drawer-close,
.ant-drawer.recharge-order-drawer .ant-drawer-close {
  color: #ffffff !important;
}

.recharge-order-drawer .ant-drawer-close:hover,
.ant-drawer.recharge-order-drawer .ant-drawer-close:hover {
  color: #4ae7fd !important;
  background: transparent !important;
}

.recharge-order-drawer .ant-drawer-close .anticon,
.ant-drawer.recharge-order-drawer .ant-drawer-close .anticon {
  color: inherit !important;
}

.recharge-order-drawer .ant-drawer-title,
.ant-drawer.recharge-order-drawer .ant-drawer-title {
  color: #ffffff !important;
}

.recharge-order-drawer .ant-drawer-header,
.ant-drawer.recharge-order-drawer .ant-drawer-header {
  background: rgba(25, 26, 29, 0.98) !important;
  border-bottom: 1px solid rgba(74, 231, 253, 0.12) !important;
}

.recharge-order-drawer .ant-drawer-content,
.ant-drawer.recharge-order-drawer .ant-drawer-content {
  background: #191a1d !important;
}

/* 充值弹窗：固定设计宽高上限，随视口收缩；首页 / 流程页同一套规则 */
.ant-modal-wrap.recharge-modal-wrap .ant-modal.recharge-main-modal {
...
.ant-modal-wrap.recharge-modal-wrap .ant-modal.recharge-pay-modal {
